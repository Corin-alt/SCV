...
import fr.corentin_owen.communications.http.handlers.TeslaConstructorHandler;
...
/**
 * Class {@link TeslaDealerShip}
 *
 * @author Corentin - Owen
 * @version 12/2021
 */
public class TeslaDealerShip {

    /**
     * Attribute(e)
     */
    private static char scenario;
    private static int teslaDealerShipPortTCP;
    ...
    public static void main(String[] args) {
        ...
        teslaDealerShipPortTCP = Config.getPort(TeslaDealerShip.class.getSimpleName(), TypePort.TCP);

        if(teslaDealerShipPortTCP == -1){
        ...
        System.out.println("TeslaDealerShip\n- HTTP port: " + serveur.getAddress().getPort() + "\n- TCP port: " + teslaDealerShipPortTCP);
        ...
        Socket socket = TCPUtils.createClientSocket("localhost", teslaDealerShipPortTCP);

        serveur.createContext("/constructor", new TeslaConstructorHandler(scenario, socket));
    ...
    public static void setPortForPHPServer(int port) {
        String name = TeslaDealerShip.class.getSimpleName().replace("DealerShip", "");
