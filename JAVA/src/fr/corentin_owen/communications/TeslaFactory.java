...
/**
 * Class {@link TeslaFactory}
 *
 * @author Corentin - Owen
 * @version 12/2021
 */
public class TeslaFactory extends Surveillant {

    /**
     * Attribute(s)
     */
    private static final String name = "TeslaFactory";

    private static final List<Parking> parkings = new ArrayList<>();

    public static int teslaFactoryPortTCP;
    public static int teslaFactoryPortUDP;
    ...
    public static void main(String[] args) {
        ...
        //-----------------------------Set port--------------------------------
        teslaFactoryPortTCP = Config.getPort(TeslaFactory.class.getSimpleName(), TypePort.TCP);
        teslaFactoryPortUDP = Config.getPort(TeslaFactory.class.getSimpleName(), TypePort.UDP);

        if (teslaFactoryPortTCP == -1 || teslaFactoryPortUDP == -1) {
        ...
        ServerSocket tcpServerSocket = TCPUtils.createServerSocket(teslaFactoryPortTCP);
        DatagramSocket udpSocket = UDPUtils.createSocket(teslaFactoryPortUDP);
        ...
        teslaProviderPortUDP = Integer.parseInt(receivePort);


        System.err.println(TeslaFactory.class.getSimpleName() + "\n- TCP: " + teslaFactoryPortTCP + "\n- UDP: " + teslaFactoryPortUDP + "\n- Provider UDP: " + teslaProviderPortUDP);

        System.out.println("\n--------------------------------------------------------------------------------\nProcessing: \n");
        //-----------------------------Initialisation Tesla factory--------------------------------
        var thread = new Thread(new TeslaFactory());
        thread.start();
                    ...
                    //construct car when provider has stock and delivery it
                        ...
                        for (String piece : pieces) {
                            ...
                            while (!stockProvider || stopInfiniteLoop > 2000) {
                                UDPUtils.sendUDPMessage(udpSocket, message.toJson().toString(), teslaProviderPortUDP);
    ...
    public static void addRandomTeslaCar() {
        Car car = null;
        while (car == null || !car.getModel().getBrand().equals("Tesla")) {
